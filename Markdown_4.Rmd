...
Formatting in scientific format:
```{r}
# Sort the table in descending order based on the savings potential
results <- results %>% arrange(desc(!!sym(name_SP)))
...
results <- results %>% arrange(desc(!!sym(name_SP_norm)))
# Create flextable
ft <- flextable(table_results) %>%
  theme_vanilla() %>%
  autofit() %>%
...
results <- results %>% arrange(desc(!!sym(name_SP_prop)))
# Create flextable 
ft <- flextable(table_results) %>%
  theme_vanilla() %>%
  autofit() %>%
